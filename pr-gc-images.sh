...
kubectl exec -it image-manager-0 -c icp-registry -n kube-system -- /bin/sh -c "find /var/lib/registry/docker/registry/v2/repositories/ -maxdepth 2 -mindepth 2 | tee /tmp/image_all" > /dev/null
kubectl exec -it image-manager-0 -c icp-registry -n kube-system -- /bin/sh -c "registry garbage-collect --dry-run /etc/docker/registry/config.yml 2>&1 | grep 'marking manifest' | cut -d ":" -f 0 | xargs -n1 echo '/var/lib/registry/docker/registry/v2/repositories/' | sed 's/ //g'| tee /tmp/image_valid" > /dev/null
